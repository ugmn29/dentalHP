...
export default function Requirements() {
  ...
  const roles: Record<RoleKey, { label: string; sub: string; basic: string; highlight: string[] }> = {
    DH: {
      ...
      highlight: ['初診P1〜SPTまで責任を持つ担当制', 'インビザライン・ワイヤー矯正の経験可', '歩合：自費メンテ・ホワイトニング 10%'],
    },
    DA: {
      label: '歯科助手・受付',
  ...
  const rows: [string, ReactNode][] = [
    ['雇用形態', '正社員（試用期間6ヶ月・賃金は基準額より5,000円減額）'],
    ['勤務地', 'Fデンタルオフィス 豊洲プライムスクエア院（変更の範囲：医院が指定する場所）'],
    ['勤務時間', <>平日 9:40 – 19:15 ／ 土日 8:40 – 17:15<br/><span className="muted">休憩 60分</span></>],
    ['休日', <>週休2日制（法定休日：火曜＋シフト週1日）／年末年始（12/29–1/3）</>],
    ['基本給', r.basic],
